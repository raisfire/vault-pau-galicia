# -*- coding: utf-8 -*-
"""Aplica los resultados del batch de clasificacion a los vault/historiaespana/*.md
(tema: [] -> tema: [...])."""
import json
import os
import re
...
batch_id = open("script/classify_historiaespana_batch_id.txt").read().strip()
targets = {t["custom_id"]: t for t in json.load(open("script/classify_historiaespana_targets.json", encoding="utf-8"))}
...
for result in client.messages.batches.results(batch_id):
    ...
    temas = [x for x in tool_use.input.get("temas", []) if x != "sin_clasificar"]
    if not temas:
        sin_clasificar += 1
    else:
        clasificadas += 1
    ...
    if temas:
        ...
        block = "tema: []"
    new_text = re.sub(r"^tema: \[\]$", block, text, count=1, flags=re.MULTILINE)
