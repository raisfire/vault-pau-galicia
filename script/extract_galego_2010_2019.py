# -*- coding: utf-8 -*-
"""Extrae los enunciados de Lingua Galega e Literatura II 2010-2019 SOLO
para estadisticas (no crea vault/). 2015 falta en las fuentes (hueco
conocido). Formato: OPCIÓN A/B, con preguntas numeradas 1-7 dentro. Para
estadisticas basta tratar cada OPCIÓN completa como una unidad a
clasificar."""
import glob
import json
import re
...
def main():
    ...
    for year in [str(y) for y in range(2010, 2020)]:
        for path in sorted(glob.glob(f"fuentes/galego/{year}/*/*.pdf")):
    ...
    with open("script/extracted_galego_2010_2019.json", "w", encoding="utf-8") as f:
